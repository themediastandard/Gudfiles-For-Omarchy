...
FEATURES = (
    ...
    Feature('browse', 'Sort & list details',
            'In list view, click a heading above the files to sort; click again to reverse. Right-click the heading row to choose Name, Size, Type, Date Created, Date Modified, Resolution, FPS, Duration or Codec. Name stays visible. Column choices and sorting are remembered. Media values load in the background; sorting by them shows progress. A dash means unavailable or not applicable. Sort in the top bar and right-click → Sort By share the same order and optional Folders first setting.',
            keywords='columns header metadata customize fps resolution created modified'),
